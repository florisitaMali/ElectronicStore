...
public class BillDAOIntegrationTest {
    ...
    public static void setupDatabase() throws Exception {
        ...
        testBill.addSoldItems(new SoldItem(testItem1.getItemName(), 5, testItem1.getSellingPrice(), testItem1.getPurchasedPrice(), testItem1.getPurchasedDate()));
        testBill.addSoldItems(new SoldItem(testItem2.getItemName(), 3, testItem2.getSellingPrice(), testItem2.getPurchasedPrice(), testItem2.getPurchasedDate()));
    }
    ...
    public void testSaveBill() {
        ...
        // Retrieve bills for today
        ArrayList<Bill> bills = BillDAO.getAllBills(LocalDate.now(), LocalDate.now());
        assertFalse(bills.isEmpty());

        boolean found = bills.stream()
                .anyMatch(b -> b.getBillNumber() == testBill.getBillNumber());
        assertTrue(found, "Saved bill should be in today's bills");
    }
    ...
    public void testBillFileCreated() {
        ...
        assertTrue(billFile.exists(), "Bill file should exist after saving bill");
    }
    ...
    public static void cleanupDatabase() throws Exception {
        ...
        try (var con = DBConnection.getConnection();
            ...
            stmt.execute("DELETE FROM bill_items WHERE bill_id IN (SELECT id FROM bills WHERE bill_number = " + testBill.getBillNumber() + ")");
            stmt.execute("DELETE FROM bills WHERE bill_number = " + testBill.getBillNumber());
        }

        // Delete test bill files
        File billFile = new File("src/main/resources/com/example/gui/bills/bill_" + testBill.getBillNumber() + ".txt");
        if (billFile.exists()) {
            billFile.delete();
        }

        // Optional: clean up test items
        ItemsDAO.softDeleteItem(testItem1);
        ItemsDAO.softDeleteItem(testItem2);
    }
}